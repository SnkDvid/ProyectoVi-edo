...
<section class="section">
    ...
    @foreach ($catalogos as $catalogo)
        <div class="grid-item" data-bs-toggle="modal" data-bs-target="#editModal{{ $catalogo->id }}">
            <img src="{{ asset('storage/' . $catalogo->id_imagen_producto) }}" alt="Imagen de catálogo">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editModalLabel{{ $catalogo->id }}">Ver Producto</h5>
            </div>
            <div class="modal-body">
                <form action="{{ route('admin.catalogo.update', $catalogo->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <img src="{{ asset('storage/' . $catalogo->id_imagen_producto) }}" alt="Imagen actual" class="img-fluid mb-2">
                    </div>

                    <!-- Nombre -->
                    <div class="mb-3">
                        <label for="new-description-{{ $catalogo->id }}" class="form-label">Nombre:</label>
                        <input type="text" class="form-control" id="new-description-{{ $catalogo->id }}" name="nombre" value="{{ $catalogo->id_nombre_producto }}" readonly>
                    </div>

                    <!-- Descripción -->
                    <div class="mb-3">
                        <label for="new-description-{{ $catalogo->id }}" class="form-label">Descripción:</label>
                        <input type="text" class="form-control" id="new-description-{{ $catalogo->id }}" name="descripcion" value="{{ $catalogo->id_descripcion }}" readonly>
                    </div>

                    <!-- Precio -->
                    <div class="mb-3">
                        <label for="new-price-{{ $catalogo->id }}" class="form-label">Precio:</label>
                        <input type="text" class="form-control" id="new-price-{{ $catalogo->id }}" name="precio" value="{{ number_format($catalogo->id_precio, 0, '', '') }}" readonly>
                    </div>

                    <!-- Peso -->
                    <div class="mb-3">
                        <label for="new-description-{{ $catalogo->id }}" class="form-label">Peso:</label>
                        <input type="text" class="form-control" id="new-description-{{ $catalogo->id }}" name="peso" value="{{ $catalogo->id_peso }}" readonly>
                    </div>

                    <!-- Categoría -->
                    <div class="mb-3">
                        <label for="product-location-{{ $catalogo->id }}" class="form-label">Donde va a ir este producto</label>
                        <select class="form-control" id="product-location-{{ $catalogo->id }}" name="categoria" disabled>
                            ...
                            <option value="pulpa" @if($catalogo->id_categoria == 'pulpa') selected @endif>3. Pulpas</option>
                        </select>
                        </div>
                            <button type="button" class="btn btn-dark" data-bs-dismiss="modal">Cerrar 🚫</button>
                            </form>
                            
                            <form action="{{ route('admin.catalogo.habilitar', $catalogo->id) }}" method="POST" class="btn_habilitar">
                            @csrf
                            @method('PUT')
